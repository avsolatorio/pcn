...
function parsePolicyFromAttrs(el: AnyNode, $: CheerioAPI): Policy {
  const type =
    (($(el).attr("policy") ?? "auto").trim() as Policy["type"]) || "auto";
  ...
  if (type === "tolerance") {
    ...
    return {
      type: "tolerance",
      tolerance: Number.isFinite(tolerance) ? tolerance : 0.02,
    };
  }
...
export function processPCNClaims(
  content: string,
